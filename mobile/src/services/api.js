...
// Using localhost for both platforms.
// For physical devices: run "adb reverse tcp:5000 tcp:5000" to tunnel.
// For emulators: change to 'http://10.0.2.2:5000' for Android.
...
/** Create a new task. */
export const createTask = async ({ title, description, status, dueDate, reminderEnabled }) => {
    const { data } = await api.post('/tasks', { title, description, status, dueDate, reminderEnabled });
    return data;
};
